    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            await i_m_s_g.edit_text(
                f"Your File Size • {d_f_s}\n"
                f"Your File • <code>{ba_se_file_name}</code> Splitted Into {number_of_files} Files.\n"
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    if os.path.isfile(file_upload):
        ...
        if INDEX_LINK:
            ...
            button.append([pyrogram.InlineKeyboardButton(text="Share Link️", url=f"{tam_link}")])
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"Hey <a href='tg://user?id={g_id}'>Buddy</a> Your File Uploaded Successfully.\n{file_upload}.\nSize • {gjay}", reply_markup=button_markup)
        ...
        if INDEX_LINK:
            ...
            tam_link = requests.utils.requote_uri(indexurl)
            LOGGER.info(tam_link)
            button.append([pyrogram.InlineKeyboardButton(text="Share Link", url=f"{tam_link}")])
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"Hey <a href='tg://user?id={g_id}'>Buddy</a> Your Folder has been Uploaded successfully to\n{tt}\nSize • {gjay}", reply_markup=button_markup)
        #await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
...
async def upload_single_file(message, local_file_name, caption_str, from_user, edit_media):
    ...
    if UPLOAD_AS_DOC.upper() == 'TRUE':
        ...
        if not edit_media:
            message_for_progress_display = await message.reply_text("Now Uploading {}".format(os.path.basename(local_file_name)))
        ...
        try:
            ...
            if not edit_media:
                message_for_progress_display = await message.reply_text(
                    "Now Uploading {}".format(os.path.basename(local_file_name))
                )
