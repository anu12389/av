...
if __name__ == "__main__" :
    ...
    incoming_youtube_playlist_dl_handler = MessageHandler(
        g_yt_playlist,
        filters=Filters.command(["ytpl@CX10BOT"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_youtube_playlist_dl_handler)
    #
    incoming_purge_message_handler = MessageHandler(
        incoming_purge_message_f,
        filters=Filters.command(["purge@CX10BOT"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_purge_message_handler)
    #
    incoming_g_clear_handler = MessageHandler(
        g_clearme,
        filters=Filters.command(["trash@CX10BOT"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    rename_message_handler = MessageHandler(
        rename_message_f,
        filters=Filters.command(["rename@CX10BOT"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(rename_message_handler)
    #
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=Filters.command(["upload@CX10BOT"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(upload_document_handler)

    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@CX10BOT"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
